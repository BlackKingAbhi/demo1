  ...
  const handleDeletePost = async () => {
    ...
    try {
      if (window.confirm("Confirm to delete the post!")) {
        ...
        if (res?.success) {
          // const updatedJobPosts = data.filter((jobPost) => jobPost._id !== job?._id);
          // setData(updatedJobPosts);
          // setRecordsCount(updatedJobPosts.length);
        ...
        <div className='w-full h-fit md:w-2/3 2xl:2/4 bg-white px-5 py-10 md:px-10 shadow-md mx-auto'>
          ...
          <div className='w-full flex items-center justify-center'>
            {user?._id === job?.company?._id ? (
              <CustomButton
                title='Delete Post'
                containerStyles={`w-50 flex items-center justify-center text-white bg-red-700 py-3 px-5 outline-none rounded-full text-base`}

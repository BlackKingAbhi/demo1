...
const Header = ({
  ...
  return (
    <div className='bg-[#f7fdfd]' style={headerBackgroundStyle}>
      <div
      ...
      >
        <div className='w-full z-10'>
          <div className='mb-8 flex items-center justify-center'>
            <p className='text-[#0e4c31] font-bold text-4xl'>{title}</p>
          </div>

          <div className='w-full flex items-center justify-around bg-white px-2 md:px-5 py-2.5 md:py-6 shadow-2xl rounded-full h-10 mb-36'>
            <SearchInput
            ...
            <div>
              <CustomButton
                ...
                title='Search'
                containerStyles={
                  "text-white py-2 md:py3 px-3 md:px-10 focus:outline-none bg-[#5f9d83] rounded-full md:rounded-md text-sm md:text-base"
                }
              />
            </div>
          </div>
          ...
          {/* {type && (
            <div className='w-full lg:1/2 flex flex-wrap gap-3 md:gap-6 py-10 md:py-14'>
              {popularSearch.map((search, index) => (
                <span
                  key={index}
                  className='bg-[#00000026] text-[#5f9d83] py-1 px-2 rounded-full text-sm md:text-base mx-auto flex flex-wrap'
                >
                  {search}
                </span>
              ))}
